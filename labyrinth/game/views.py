...
def safe_get_state(game, pid):
    """
    BULLETPROOF WRAPPER: Handles uWSGI memory cache drift.
    This dynamically tests the models.py signature currently stuck in RAM
    so the server never crashes, regardless of which version is loaded.
    """
    try:
        # Try the most recent update
        return game.get_state_dict(player_id=pid)
    except TypeError:
        try:
            # Try the perspective update
            return game.get_state_dict(player_perspective=pid)
        except TypeError:
            # Fallback to the original base version
...
def board(request, labyrinth_id):
    ...
    return render(request, 'game/board.html', {
        ...
        'is_host': is_host,
        'theme': game.theme,
        'state_json': json.dumps(safe_get_state(game, player_id)),
    })
...
def poll_state(request, labyrinth_id):
    game = get_object_or_404(Game, pk=labyrinth_id)
    player_id = request.session.get(f'player_id_{game.id}', 0) if game.mode == 'online' else 0
    return JsonResponse(safe_get_state(game, player_id))
...
def api_rotate_spare(request, labyrinth_id):
    ...
    return JsonResponse(safe_get_state(game, pid))
